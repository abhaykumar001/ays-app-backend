<x-app-layout>
    ...
    <div class="py-6">
        <div class="sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('user.update', $user->id) }}" class="mt-6 space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid md:grid-cols-12 gap-5">
                        ...
                        <!-- Password -->
                        <div class="md:col-span-6">
                            <x-input-label for="password" :value="__('New Password (Leave blank to keep existing)')" />
    ...
    <!-- JS for module select -->
    <script>
        document.querySelectorAll('.module-checkbox').forEach(moduleCheckbox => {
            moduleCheckbox.addEventListener('change', function() {
                const moduleClass = `.module-${this.id.replace('module_', '')}`;
                document.querySelectorAll(moduleClass).forEach(perm => {
