<x-app-layout>
    <div x-data="unitMediaModal()" x-init="@if ($errors->any()) show = true @endif">

        <x-slot name="header">
            <x-nav-link :href="route('projects.units.index', $unit->project->id)" :active="true">
                Virtual Tours – {{ $unit->title }}
            </x-nav-link>
        </x-slot>
        ...
        <div class="py-6">
            <div class="bg-white dark:bg-gray-800 p-6 rounded shadow">
                ...
                @php
                    ...
                    if (auth()->user()->can('edit_unit_media')) {
                        $actions[] = ['type' => 'edit', 'label' => 'Edit', 'click' => true];
                    }

                    if (auth()->user()->can('delete_unit_media')) {
                        $actions[] = [
                            'type' => 'delete',
                            'url' => 'units.unitMedia.destroy',
                            'params' => [$unit->id],
                            'label' => 'Delete',
                        ];
                    }
                ...
                <x-datatable :data="$unitMedia" :columns="$columns" />
        ...
        <script>
            function unitMediaModal() {
                return {
                    ...
                    isEdit: false,
                    unitId: {{ $unit->id }},
                    createUrl: "{{ route('units.unitMedia.store', $unit) }}",
                    updateUrl: "",
                    filePreview: null,
                    form: {
                        title: '',
                    ...
                    openEdit(id) {
                        this.isEdit = true;
                        this.show = true;
                        this.updateUrl = `/dashboard/units/${this.unitId}/unitMedia/${id}`;

                        fetch(`/dashboard/units/${this.unitId}/unitMedia/${id}/edit`, {
                                headers: {
