                ...
                <div class="grid md:grid-cols-2 gap-4 mb-4">
                    <div class="my-auto">
                        <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                            {{ __('Edit Offer') }}: {{ $projectOffer->title }}
                        </h2>
                    </div>
                </div>

                <form method="POST" action="{{ route('projects.projectOffers.update', [$project, $projectOffer]) }}" class="mt-6 space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid md:grid-cols-12 gap-5">
                        ...
                        <div class="md:col-span-6">
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full"
                                :value="old('title', $projectOffer->title)" required autofocus />
                        ...
                        <div class="md:col-span-3">
                            ...
                            ]" :value="old('category', $projectOffer->category)" />
                        ...
                        <div class="md:col-span-3">
                            ...
                            <x-select name="is_active" :options="['1' => 'Active', '0' => 'Inactive']"
                                :value="old('is_active', $projectOffer->is_active ? '1' : '0')" />
                        ...
                        <div class="md:col-span-12">
                            ...
                            <x-text-textarea id="description" name="description" class="mt-1 block w-full">{{ old('description', $projectOffer->description) }}</x-text-textarea>
                        ...
                        <div class="md:col-span-3">
                            ...
                            <x-text-input id="sort_order" name="sort_order" type="number" class="mt-1 block w-full"
                                :value="old('sort_order', $projectOffer->sort_order)" />
                            ...
                            <table class="w-full text-sm text-left">
                                ...
                                <tbody>
                                    @forelse ($units as $unit)
                                        @php
                                            $offerUnit = $offerUnits->get($unit->id);
                                        @endphp
                                        <tr class="border-b border-gray-600">
                                            <td class="px-4 py-3">
                                                <input type="checkbox" name="units[{{ $unit->id }}][included]" value="1"
                                                    {{ old('units.' . $unit->id . '.included', $offerUnit ? '1' : '') ? 'checked' : '' }} />
                                            </td>
                                            ...
                                            <td class="px-4 py-3">
                                                <x-text-input name="units[{{ $unit->id }}][price]" type="number" step="0.01"
                                                    ...
                                                    :value="old('units.' . $unit->id . '.price', $offerUnit?->price)"
