<x-app-layout>
    <x-slot name="header">
        ...
        <x-nav-link :href="route('locations.edit', $location)" :active="true">Edit Location</x-nav-link>
    </x-slot>

    <div class="py-6">
        <div class="sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-6">Edit Location</h2>
                ...
                <form method="POST" action="{{ route('locations.update', $location) }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf @method('PUT')
                    <div class="grid md:grid-cols-12 gap-5">
                        ...
                        <div class="md:col-span-4">
                            ...
                            <x-text-input id="sort_order" name="sort_order" type="number" min="0"
                                class="mt-1 block w-full" :value="old('sort_order', $location->sort_order)" />
                        ...
                        <div class="md:col-span-12">
                            ...
                            <x-text-textarea id="description" name="description" class="mt-1 block w-full" required>{{ old('description', $location->description) }}</x-text-textarea>
                        ...
                        <div class="md:col-span-6 flex items-center">
                            <label for="is_active" class="flex items-center gap-2 mt-6 cursor-pointer">
                                <input type="checkbox" id="is_active" name="is_active" value="1"
                                    {{ old('is_active', $location->is_active ? '1' : '0') == '1' ? 'checked' : '' }}
                        ...
                        <div class="md:col-span-4">
                            ...
                            <x-text-input id="website_url" name="website_url" type="url" class="mt-1 block w-full"
                                :value="old('website_url', $location->website_url)" />
                        ...
                        @foreach (['monday' => 'Monday', 'tuesday' => 'Tuesday', 'wednesday' => 'Wednesday', 'thursday' => 'Thursday', 'friday' => 'Friday', 'saturday' => 'Saturday', 'sunday' => 'Sunday'] as $day => $label)
                            <div class="md:col-span-3">
                                ...
                                <x-text-input id="opening_hours_{{ $day }}" name="opening_hours[{{ $day }}]" type="text"
                                    ...
                                    :value="old('opening_hours.' . $day, data_get($location->opening_hours, $day))" />
                        ...
                        <div class="md:col-span-12">
                            ...
                            <x-text-input id="address" name="address" type="text"
                                class="mt-1 block w-full map-input" :value="old('address', $location->address)" />
                            <x-text-input type="hidden" name="latitude" id="latitude" value="{{ old('latitude', $location->latitude ?? 0) }}" />
                            <x-text-input type="hidden" name="longitude" id="longitude" value="{{ old('longitude', $location->longitude ?? 0) }}" />
                    ...
                    <div class="flex items-center gap-4">
                        <x-primary-button>Update Location</x-primary-button>
